...
class FavoritesDataProviderObserver extends ContentObserver {
    ...
    FavoritesDataProviderObserver(AppWidgetManager mgr, ComponentName cn, Handler h) {
        super(h);
    ...
    public void onChange(boolean selfChange) {
        ...
        mAppWidgetManager.getAppWidgetIds(mComponentName), R.id.mylist);
...
public class BookmarkWidgetProvider extends AppWidgetProvider {
    ...
    private static FavoritesDataProviderObserver sDataObserver;
    ...
    public void onEnabled(Context context) {
        ...
        if (sDataObserver == null) {
            ...
            final ComponentName cn = new ComponentName(context, BookmarkWidgetProvider.class);
            sDataObserver = new FavoritesDataProviderObserver(mgr, cn, sWorkerQueue);
            r.registerContentObserver(BookmarkDataProvider.CONTENT_URI, true, sDataObserver);
          //test//
    ...
    public void onReceive(Context ctx, Intent intent) {
        ...
        if (action.equals(REFRESH_UPDATE_ACTION)) {
            ...
            sWorkerQueue.post(new Runnable() {
                ...
                public void run() {
                    final ContentResolver r = context.getContentResolver();
                    final Cursor c = r.query(BookmarkDataProvider.CONTENT_URI, null, null, null, 
                            null);
                    ...
                    BookmarkDataProvider.what=what;
                	...
                	//v. 2.2:
                	BookmarkDataProvider.appwidgetid=appWidgetId;
                	
                	//Toast.makeText(context, context.getText(R.string.refresh), Toast.LENGTH_SHORT).show();
                    mgr.notifyAppWidgetViewDataChanged(appWidgetId, R.id.mylist);//mgr.getAppWidgetIds(cn)
        	...
        	MainActivity.typeofwidget="4x2";//v. 2.2
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
       ...
       // context.startService(new Intent(context, BookmarkWidgetService.class));
        ...
        for (int i = 0; i < appWidgetIds.length; ++i) {
        	...
        	final Intent refreshIntent = new Intent(context, BookmarkWidgetProvider.class);
            refreshIntent.setAction(BookmarkWidgetProvider.REFRESH_UPDATE_ACTION);
	...
	static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
    	...
    	final Intent intent = new Intent(context, BookmarkWidgetService.class);
      ...
      //v. 2.2:
    	...
    	if (theme.equals("dark")){
    		...
    		if (back.equals("high")){
        		whichBackground=R.layout.layout_appwidget_trans_white;
        	}else if (back.equals("low")){
        		whichBackground=R.layout.layout_appwidget_white;
        	}else if (back.equals("full")){//v. 1.7
        		whichBackground=R.layout.layout_appwidget_full_trans_white;
        	}else if (back.equals("none")){//v. 1.7
        		whichBackground=R.layout.layout_appwidget_notrans_white;
        	}
    	}else{//custom v.1.7
    		whichBackground=R.layout.layout_appwidget_notrans_custom;//v. 1.8
    	}
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
        	rv.setRemoteAdapter(R.id.mylist, intent);
        }else{
        	rv.setRemoteAdapter(appWidgetId, R.id.mylist, intent);
        }
        // Set the empty view to be displayed if the collection is empty.  It must be a sibling
        // view of the collection view.
        rv.setEmptyView(R.id.mylist, R.id.empty_view);
        ...
        if (theme.equals("custom")){//v.1.7:get custom colors
        	...
        	rv.setTextColor(R.id.textView1, colorText);
        ...
        // ignored otherwise.
        final Intent onClickIntent = new Intent(context, BookmarkWidgetProvider.class);
        onClickIntent.setAction(BookmarkWidgetProvider.CLICK_ACTION);
        onClickIntent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId);
        onClickIntent.setData(Uri.parse(onClickIntent.toUri(Intent.URI_INTENT_SCHEME)));
        final PendingIntent onClickPendingIntent = PendingIntent.getBroadcast(context, 0,
                onClickIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        rv.setPendingIntentTemplate(R.id.mylist, onClickPendingIntent);//listbutton
        
        

        // Bind the click intent for the refresh button on the widget
        final Intent refreshIntent = new Intent(context, BookmarkWidgetProvider.class);
        refreshIntent.setAction(BookmarkWidgetProvider.REFRESH_ACTION);
////////////////
        Uri data = Uri.withAppendedPath(
     ...
     // Bind the click intent for the search button on the widget
       ...
       final Intent searchIntent = new Intent(context, BookmarkWidgetProvider.class);
        
        searchIntent.setData(data);
        searchIntent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId);
        searchIntent.setAction(BookmarkWidgetProvider.SEARCH_ACTION);
        ...
        rv.setOnClickPendingIntent(R.id.textView1, searchPendingIntent);
